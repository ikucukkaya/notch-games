...
The right-mounted hoop uses a broadcast-style side profile. A padded vertical backboard sits near the screen edge with a steel wall plate, hinge, and diagonal braces behind it; a boxed orange support arm and lower mounting bracket extend leftward from the board before meeting the rim. The projected rim is a shallow ellipse split into rear and front render layers, so the basketball appears in front of the far edge and behind the near edge while passing through. Dual-tone contrast edges pair dark silhouettes with bright inner strokes so the complete assembly remains readable over both white documents and dark desktops without sampling the screen. Two small circular rim colliders leave the opening physically clear, and the thin vertical board collider supports bank shots without creating a horizontal shelf.

The procedural diamond-weave net is driven by a 121-knot Verlet cloth simulation with eight iterative constraint passes per substep. Its pinned top cord follows the rim's projected ellipse, low-stiffness bending links spread impacts smoothly, and explicit row-order constraints prevent lower knots from crossing or remaining tangled. Alternating cord families render behind and in front of the ball; curved visual spans use the hidden intermediate simulation rows, preserving the smoother motion without making the net look like a dense cage. Swept contact reacts to brushes from above, either side, and underneath even when the shot does not score. A continuous tapered funnel constraint activates only when a descending ball enters through the rim, absorbs lateral speed, prevents the ball from escaping through a diamond opening, and releases it only after it clears the bottom of the net. The ball also collides with screen edges and the inferred floor.
